...
module.exports = {
    ...
    name: "join",
    description: "Active ou desactive le message d'arriver",
    ...
    async run(bot, interaction) {
        ...
        const on_embed = new Discord.EmbedBuilder()
            .setColor(color)
            .setDescription(`✅ ${interaction.user.tag} le message de join est bien activé !`)
        const off_embed = new Discord.EmbedBuilder()
            .setColor(color)
            .setDescription(`✅ ${interaction.user.tag} le message de join est bien desactivé !`)
        ...
        if(on_off == "on") {
            ...
            await db.set("join" + interaction.guild.id,true)
        }
        else if(on_off == "off") {
            interaction.reply({ embeds: [off_embed]})
            await db.set("join" + interaction.guild.id,false)
        }else {     
