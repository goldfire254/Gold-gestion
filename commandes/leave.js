...
module.exports = {
    ...
    name: "leave",
    description: "Active ou desactive le message de leave",
    ...
    async run(bot, interaction) {
        ...
        const on_embed = new Discord.EmbedBuilder()
            .setColor(color)
            .setDescription(`✅ ${interaction.user.tag} le message de leave est bien activé !`)
        const off_embed = new Discord.EmbedBuilder()
            .setColor(color)
            .setDescription(`✅ ${interaction.user.tag} le message de leave est bien desactivé !`)
        ...
        if(on_off == "on") {
            ...
            await db.set("leave" + interaction.guild.id,true)
        }
        else if(on_off == "off") {
            interaction.reply({ embeds: [off_embed]})
            await db.set("leave" + interaction.guild.id,false)
        }else {     
