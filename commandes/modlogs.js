...
module.exports = {
    ...
    name: "modlogs",
    description: "Active les logs de modérateur",
    ...
    async run(bot, interaction) {
        ...
        const on_embed = new Discord.EmbedBuilder()
            .setColor(color)
            .setDescription(`✅ ${interaction.user.tag} modlogs est bien activé !`)
        const off_embed = new Discord.EmbedBuilder()
            .setColor(color)
            .setDescription(`✅ ${interaction.user.tag} modlogs est bien desactivé !`)
        ...
        if(on_off == "on") {
            ...
            await db.set("modlogs" + interaction.guild.id,true)
        }
        else if(on_off == "off") {
            interaction.reply({ embeds: [off_embed]})
            await db.set("modlogs" + interaction.guild.id,false)
        }else {     
